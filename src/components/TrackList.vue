<template>
  <div class="track-list">
    ...
    <ul>
      ...
      >
        <span v-if="editId !== t.id" class="name-label" @dblclick.stop="startEdit(t)">{{ t.name }}</span>
        <input
          v-else
          class="name-input"
          v-model="editName"
...
span.name-label { flex: 1; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
input.name-input { flex: 1; border: 1px solid #cfd6ea; border-radius: 4px; padding: 3px 6px; }
button.small {
  font-size: 12px;
  padding: 4px 8px;
}
